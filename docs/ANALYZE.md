...
| `has_doc` | A doc comment, in each language's own form: Rust `///`, JSDoc `/** */`, Javadoc, and — from schema version 8 — a Python docstring. Before 8 `python.rs` used the JSDoc scanner, which looks *above* the node, so every Python symbol reported 0 and `doc_coverage` answered "0% documented" for codebases that were not. `graph_schema` reports a pre-8 graph as `stale_python_docs`. |
...
| `params` / `max_nesting` / `members` | `members` counts a type's declared members: a class body's fields and methods, and — contrary to what this table said for a long time — a Rust type's `impl` blocks, which the graph builder attaches to the type (107 of this repo's 311 Rust types carry a count). Absent, not zero, for a type nobody declared members for. |
