...
│  → search_kb({query, k:5}) → get_code on best hit
│
├─ "What files are in module X?"
│  → file_outline({file: "path/to/module"})
│
├─ "Read this function's source"
│  → get_code({nodeId})  (use nodeId from find_symbol/search_kb/file_outline)
│
├─ "Who calls / imports this?"
│  → find_usages({nodeId})
│
├─ "What does this function depend on?"
│  → traverse_kb({nodeId, direction:"outbound", hops:1})
│
...
search_kb({ query: "main indexing pipeline entrypoint", k: 5 })
...
traverse_kb({ nodeId: "function:native/src/indexer/mod.rs:42:index_project", direction: "outbound", hops: 1 })
...
search_kb({ query: "test commands scripts", k: 3 })
```
...
search_kb({ query: "MCP server start initialization", k: 5 })

# Step 2: read the main function
get_code({ nodeId: "function:native/src/main.rs:10:main" })

# Step 3: see startup dependencies
traverse_kb({ nodeId: "function:native/src/main.rs:10:main", direction: "outbound", hops: 2 })
```
...
search_kb({ query: "file discovery walker finds files in directory", k: 5 })
...
traverse_kb({ nodeId: "function:native/src/indexer/mod.rs:120:index_file", direction: "outbound", hops: 1 })
```
...
search_kb({ query: "use old_crate_name", k: 15 })
...
- **Don't** `search_kb` for a known name → use `find_symbol({name})` (50-100x cheaper tokens).
  Example: instead of `search_kb("the for_extension function")`, do `find_symbol({name:"for_extension"})`.
...
- **Don't** `search_kb` for file structure → use `file_outline({file})`.
  Example: instead of `search_kb("what functions are in languages.rs")`, do `file_outline({file:"native/src/indexer/languages.rs"})`.
...
- **Don't** `traverse_kb` at `hops: 3` by default → start at `hops: 1`, expand if needed.
- **Don't** `search_kb` with `k: 20` on first try → start at `k: 5`, then expand with `get_code` on promising hits.
- **Don't** read code from search snippets alone → use `get_code` for full context.
  Example: search_kb shows 20 lines of `for_extension` — still call `get_code` to see the whole function.
