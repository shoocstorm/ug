//! Turning an [`IndexResult`](crate::types::IndexResult) into a
//! [`GraphData`] — nodes, edges, and the name resolution that connects them.
//!
//! This is the half of `graph` that *writes* the graph. Nothing here reads a
//! finished graph back; that is [`super::algos`]. The two share no state, only
//! the [`GraphData`] shape they hand between them.
//!
//! The hard part is resolution: a call site names `parse`, and the repo has
//! three of them. [`QualifiedIndex`] and the fallbacks around it decide which
//! one an edge points at, and [`ResolutionStats`](crate::types::ResolutionStats)
//! records how confident each decision was.
...
/// the top of every pass so the bodies read against plain names. Only fields
/// that actually cross a pass boundary live here — `route_ids`, the dedup
/// guard for `route:` nodes, stays a local in the pass that owns it.
#[derive(Default)]
struct GraphAccum {
    nodes: Vec<GraphNode>,
    edges: Vec<GraphEdge>,
    ...
    symbol_id_map: HashMap<String, Vec<String>>,
    ...
    constant_ids: HashSet<String>,
    /// Written only by [`resolve_call_edges`]: call resolution is the one
    /// step that can be confidently right, guessing, or give up.
    resolution: crate::types::ResolutionStats,
}

/// The two lookup tables built once from the file list and read by the passes
/// that resolve a path or a bare basename back to a file node.
struct FileIndexes {
    path: HashMap<String, String>,
    basename: HashMap<String, Vec<String>>,
}

/// Build the whole graph from one indexing run.
///
/// Five sequential passes over the same [`GraphAccum`]. Order is load-bearing:
...
pub(super) fn build_graph_from_index(index_result: &crate::types::IndexResult) -> GraphData {
    ...
    let dependency_ids = add_dependency_nodes(index_result, &mut acc);
    add_folder_nodes(index_result, &files, &mut acc);
...
/// Pass 1 — declared third-party dependencies, as `GraphNodeType::Dependency`
/// nodes. Returns the spec -> node id map pass 5 needs to point unresolved
/// imports at a package rather than dropping them.
fn add_dependency_nodes<'a>(
    index_result: &'a crate::types::IndexResult,
    acc: &mut GraphAccum,
) -> HashMap<&'a str, String> {
    let GraphAccum { nodes, .. } = acc;
...
/// Pass 2 — the folder forest, so a retriever can climb from a leaf file to
...
fn add_folder_nodes(
    ...
    let GraphAccum { nodes, edges, .. } = acc;
    ...
    for f in &index_result.folders {
        ...
        // files.path (rather than format!("file:{}", path)) so a file that
...
/// Pass 3 — every file and symbol node, and the name indexes the two
/// resolution passes below look names up in.
fn add_file_and_symbol_nodes(index_result: &crate::types::IndexResult, acc: &mut GraphAccum) {
    let GraphAccum { nodes, edges, symbol_id_map, qualified, constant_ids, .. } = acc;
    ...
    // a path) and should meet at one node. Never leaves this pass.
...
/// Pass 4 — resolve calls/extends/implements into edges. Needs the indexes
/// pass 3 filled.
fn resolve_call_edges(index_result: &crate::types::IndexResult, acc: &mut GraphAccum) {
    let GraphAccum { edges, symbol_id_map, qualified, constant_ids, resolution, .. } = acc;
...
/// Pass 5 — resolve file-level imports against the file index. Needs both
/// the indexes from pass 3 and the dependency nodes from pass 1.
fn resolve_import_edges(
    index_result: &crate::types::IndexResult,
    files: &FileIndexes,
    dependency_ids: &HashMap<&str, String>,
    acc: &mut GraphAccum,
) {
    let GraphAccum { edges, symbol_id_map, qualified, .. } = acc;
